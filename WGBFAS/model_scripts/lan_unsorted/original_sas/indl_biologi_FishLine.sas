...
if aar=2023 and area='3as' and art='SIL' and kat='KON' and kv=1 and lgdcm=12 then do;
  lgdcm=11; manual_imp = 'yes'; output;
end;
...
proc sql;
create table lan as
select year as aar, new_area as area, quarter as kv, kategori as kat, art, sum(ton*1000) as kg, sum(ton) as ton
from spr.lan_&aar.
group by aar, area, kv, kat, art;
